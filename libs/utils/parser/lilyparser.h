...
namespace ExPop {
    ...
    /// Parser node. Stores information in a hierarchy and can be saved to
    /// or read from text.
    class ParserNode {

        public:

            /// Get the name of this node. Top-level nodes will be
            /// named "root" by default.
            std::string getName(void);

            /// Get the value for this as a boolean. Returns true if
            /// the value is defined, otherwise returns false and does
            /// not set the value.
            bool getBooleanValue(const std::string &name, bool *value);

            /// Get the value for this as a string. Returns true if
            /// the value is defined, otherwise returns false and does
            /// not set the value.
            bool getStringValue(const std::string &name, std::string *value);

            /// Get the value for this as a float. Returns true if the
            /// value is defined, otherwise returns false and does not
            /// set the value.
            bool getFloatValue(const std::string &name, float *value);

            /// Get the value for this as an integer. Returns true if
            /// the value is defined, otherwise returns false and does
            /// not set the value.
            bool getIntValue(const std::string &name, int *value);

            /// Decode a binary buffer from the string, treating it as
            /// a string of hex values for bytes. Calling function
            /// owns the buffer and is responsible for cleaning it up.
            bool getBinaryValue(const std::string &name, char **buf, int *length);

            /// Get a string value, but we don't care if it fails (and
            /// just returns an empty string.
            std::string getStringValueDirty(const std::string &name);

            /// Encode a buffer as hex values and store it as a value
            /// on the node.
            ...
            /// Set a value that can be written to a normal ostream
            /// and read from an istream.
            template<class T>
            ...
            /// Add a child to the start of the list of children
            /// (warning: slow).
            void addChildToStart(ParserNode *node);

            /// Output this ParserNode and all its children to an
            /// output stream in a format readable by parseBuffer()
            /// and parseString().
